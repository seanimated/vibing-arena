...
/**
 * Resolve which admin is performing the current request, for audit attribution.
 * Looks at the session first (if the app maintains one), then an explicit
 * admin_id sent by the frontend (pulled from its logged-in user_data), falling
 * back to the seeded system admin (id 1) if nothing else is available.
 */
function current_admin(PDO $pdo, array $body = []): array
{
    ...
    if ($id) {
        ...
        ", [':id' => $id]);
        if ($row) {
            return ['id' => (int) $row[0]['id'], 'email' => $row[0]['email'], 'role' => $row[0]['role_name']];
        }
    }

    // Fallback: seeded system admin — keeps existing behaviour when no
    // session/admin_id is available, but attribution should be preferred
    // wherever the frontend can supply admin_id.
    return ['id' => 1, 'email' => 'admin@tenderportal.co.za', 'role' => 'super_admin'];
}

/**
 * Write a row to audit_logs. Never throws — a logging failure should not
 * break the primary action it's attached to.
 */
function audit_log(PDO $pdo, array $e): void
{
    try {
        $stmt = $pdo->prepare("
            INSERT INTO audit_logs
                (actor_type, actor_id, actor_email, actor_role, action,
                 entity_type, entity_id, entity_label, old_data, new_data,
                 change_summary, ip_address, request_url, request_method, result, failure_reason)
            VALUES
                (:actor_type, :actor_id, :actor_email, :actor_role, :action,
                 :entity_type, :entity_id, :entity_label, :old_data, :new_data,
                 :change_summary, :ip_address, :request_url, :request_method, :result, :failure_reason)
        ...
        $stmt->execute([
            ...
            ':old_data'       => isset($e['old_data']) ? json_encode($e['old_data']) : null,
            ':new_data'       => isset($e['new_data']) ? json_encode($e['new_data']) : null,
            ':change_summary' => $e['change_summary'] ?? null,
...
//
// AUDIT: every push is attributed to the resolved admin (current_admin()) and
// logged per-tender to audit_logs as a 'raw_tender.pushed' event, so the Audit
// Trail can show exactly who pushed each tender and with what result.
// ══════════════════════════════════════════════════════════════════════════════
function action_push_nts(PDO $pdo, array $body): never
{
    ...
    // Resolve who is performing this push, for audit attribution
    $admin   = current_admin($pdo, $body);
    $idsList = array_values($ids);

    // Snapshot each tender's current push status/title before the push, so
    // the audit log can show the before/after state
    $preById = [];
    $byRef   = [];
    if ($idsList) {
        $ph = implode(',', array_fill(0, count($idsList), '?'));
        foreach (rows($pdo, "
            SELECT id, title, reference_number, nts_push_status
            FROM published_tenders
            WHERE id IN ($ph)
        ", $idsList) as $pr) {
            $pid = (int) $pr['id'];
            $preById[$pid] = $pr;
            if (!empty($pr['reference_number'])) {
                $byRef[$pr['reference_number']] = $pid;
            }
        }
    }

    $result     = NtsApi::pushBatch($pdo, $idsList, $admin['id']);
    $resultRows = $result['results'] ?? [];

    // Log one audit row per tender in the push result, attributing the
    // action to the resolved admin so the Audit Trail can show who pushed
    // each tender and whether it succeeded, duplicated, or failed.
    foreach ($resultRows as $i => $r) {
        $num   = $r['number'] ?? $r['tender_number'] ?? null;
        $pubId = ($num !== null && isset($byRef[$num])) ? $byRef[$num] : ($idsList[$i] ?? null);
        if ($pubId === null) continue;

        $status      = $r['status'] ?? null;
        $pre         = $preById[$pubId] ?? null;
        $auditResult = match ($status) {
            'success'   => 'success',
            'duplicate' => 'warning',
            'failed'    => 'failure',
            default     => 'success',
        };
        $label = $pre['title'] ?? $num ?? ('#' . $pubId);
        ...
        audit_log($pdo, [
            ...
            'action'         => 'raw_tender.pushed',
            'entity_type'    => 'published_tenders',
            'entity_id'      => $pubId,
            'entity_label'   => $label,
            'old_data'       => $pre ? ['nts_push_status' => $pre['nts_push_status']] : null,
            'new_data'       => [
                'nts_push_status' => $status ?? 'unknown',
                'nts_uid'         => $r['uid'] ?? $r['nts_uid'] ?? null,
            ],
            ...
            'result'         => $auditResult,
            'failure_reason' => $status === 'failed' ? ($r['message'] ?? null) : null,
        ]);
    }
